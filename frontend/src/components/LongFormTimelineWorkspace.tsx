...
export function LongFormTimelineWorkspace({
  ...
  const appendUploadedVideos = async (files: File[]) => {
    ...
    try {
      ...
      if (inserted.length) onDispatch({
        ...
        anchorId,
        position: "after",
        newIds: inserted.map(() => createSegmentId()),
      });
  ...
  const detectActiveSegmentShots = async () => {
    ...
    try {
      ...
      onDispatch({
        ...
        expected,
        newIds: appliedCuts.map(() => createSegmentId()),
      });
  ...
  const handleTimelineKey = (event: ReactKeyboardEvent) => {
    ...
    if (command && event.key.toLocaleLowerCase() === "d") {
      ...
      const copyCount = Math.min(
        ...
        Math.max(0, 128 - state.project.segments.length),
      );
      onDispatch({
        type: "segment/duplicate-selected",
        newIds: Array.from({ length: copyCount }, () => createSegmentId()),
      });
      return;
    }
  ...
  const handleTimelineAssetDrop = (
    ...
    if (firstVideo) onDispatch({
      type: "segment/insert-video",
      asset: firstVideo,
      anchorId: state.active_segment_id ?? state.selected_segment_ids.at(-1) ?? null,
      position: "after",
      newId: createSegmentId(),
    });
    assetDropTargetRef.current = null;
    ...
    <main className="longform-workspace" aria-label="长视频时间线工作区">
      ...
      <section className="timeline-commandbar" aria-label="时间线功能按钮区">
        ...
        <div className="timeline-commandbar__group timeline-commandbar__group--edit">
          ...
          <button type="button" disabled={state.project.segments.length >= 128} onClick={() => onDispatch({ type: "segment/insert", position: "before", mode: activeSegment?.mode ?? "fl2va", newId: createSegmentId() })}>＋ 前插空段</button>
          <button type="button" disabled={state.project.segments.length >= 128} onClick={() => onDispatch({ type: "segment/insert", position: "after", mode: activeSegment?.mode ?? "fl2va", newId: createSegmentId() })}>＋ 后插空段</button>
          ...
          <button type="button" disabled={!state.selected_segment_ids.length || state.project.segments.length >= 128} onClick={() => onDispatch({ type: "segment/duplicate-selected", newIds: Array.from({ length: Math.min(selectedSegments.length, Math.max(0, 128 - state.project.segments.length)) }, () => createSegmentId()) })}>复制片段</button>
