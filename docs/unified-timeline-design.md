...
- 操作栏：插入、分割、复制、合并、删除。片段整块手势与右上复选框共同维护唯一分段选择；标题旁“全选”包含
  启用和停用段，“禁用所选”集中修改启用状态，实际预检与生成范围恒为 `selected ∩ enabled`。
- 主时间线：唯一主轨，无重叠。首次绑定或拖入 Ref2VA 源视频时，以服务器探测的完整时长铺到主轨，
  ...
  Ctrl/Command 切换选择、Shift 按当前启用/停用轨的可见顺序范围选择及键盘删除；跨轨 Shift 从目标段重新起选，
  不会顺带选择夹在主轨片段之间的停用段。
